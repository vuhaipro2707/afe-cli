...
_build_fix_prompt() {
  ...
  if [[ -z "\$last_cmd" ]]; then
    ...
    if [[ "\$last_cmd" == ($CMD_F|$CMD_FL|$CMD_FLT|$CMD_FE|$CMD_A|$CMD_AL|$CMD_ALT|$CMD_E|$CMD_EL|$CMD_Q|$CMD_QL|$CMD_QLT|afe-help|ai-help|ai)\ * || "\$last_cmd" == ($CMD_F|$CMD_FL|$CMD_FLT|$CMD_FE|$CMD_A|$CMD_AL|$CMD_ALT|$CMD_E|$CMD_EL|$CMD_Q|$CMD_QL|$CMD_QLT|afe-help|ai-help|ai) ]]; then
...
$CMD_Q() {
  ...
  local prompt="\$(_build_query_prompt "\$*")"
  local sys_prompt="You are a high-efficiency terminal consultant and DevOps engineer.
