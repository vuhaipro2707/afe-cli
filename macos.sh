...
2. STRICTLY PROHIBITED: Do NOT use markdown code blocks (NO \`\`\` or \`\`\`sh). Do NOT include any explanations, notes, conversational filler, or quotes.
...
1. When mode is [ERROR FIX MODE]:
   ...
   - Do NOT use markdown code blocks (NO \`\`\` or \`\`\`sh).
...
_build_fix_prompt() {
  ...
  if [[ -z "\$last_cmd" ]]; then
    ...
    if [[ "\$last_cmd" == ($CMD_F|$CMD_FL|$CMD_FLT|$CMD_FE|$CMD_A|$CMD_AL|$CMD_ALT|$CMD_E|$CMD_EL|afe-help|ai-help|ai)\ * || "\$last_cmd" == ($CMD_F|$CMD_FL|$CMD_FLT|$CMD_FE|$CMD_A|$CMD_AL|$CMD_ALT|$CMD_E|$CMD_EL|afe-help|ai-help|ai) ]]; then
...
Output ONLY the exact raw fixed command. No markdown."
  else
...
User question: \$query
Answer the user question directly based on the output in \${AI_RESPONSE_LANG:-$AI_RESPONSE_LANG}."
  fi
}
...
$CMD_F() {
  [[ "\$1" == "-h" || "\$1" == "--help" ]] && { afe-help; return 0; }
  local prompt="\$(_build_fix_prompt "\$*")"
  local sys_prompt="You are a macOS terminal command generator. Output ONLY the single raw executable shell command to fix the issue or satisfy the request. No markdown, no backticks, no explanations, no conversational text."
  local tmp_file=\$(mktemp)
...
- Format:
  ...
  2. Fix: The exact terminal command(s) needed.
